...
| `simplelock.mongo.enabled`           | Whether the JDBC lock is enabled                              | `true`
...
Add the project dependency into your `pom.xml` for SQL:
[source,xml]
...
Add the project dependency into your `build.gradle` for SQL:
[source,groovy]
...
Or add the project dependency into your `build.gradle` for NoSQL:
[source,groovy]
