...
or another approach, where you need to release the lock later on in another service e.g.
...
| `com.github.simplelock.jdbc.auto-generate-ddl` | Whether to automatically create the DB object(s) | `true`
| `com.github.simplelock.jdbc.cleanup-on-startup` | Whether to clean up DB if any existing locks are not released | false
|===
...
[source,groovy]
----
implementation('com.github.natureknight.simplelock:simplelock-spring-starter:1.2.6')
----
